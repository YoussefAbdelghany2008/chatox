...
function Input({children, placeholder, name , isPasswordFeild = false,autoFocus=false}) {
    ...
    return (
            <div className="flex h-12">
                <label htmlFor={name} className={`h-full rounded-l w-14 center bg-current text-white`}>{children}</label>
                <div className={`flex items-center w-full relative`} >
                    <input id={name} name={name} autoFocus={autoFocus}
                        ...
                        className={`${isPasswordFeild ? 'pr-7' : ''} peer size-full font-medium text-base rounded-r-md px-3 py-2 border-2 bg-white shadow-sm focus:border-current focus:outline-none block`}/>
                    <label htmlFor={name} className={`absolute font-semibold text-base left-4 ${isTyped ? `bg-white -top-2 font-medium text-xs peer-focus:text-current text-slate-500` : `text-slate-500`}`}>{placeholder}</label>
                    {isPasswordFeild ? (
